...
const int MAX = 10; // Tamano de la matriz
const int CELL_SIZE = 48; // Tamano de cada celda
const int SCREEN_W = MAX * CELL_SIZE + 250; // Ancho de ventana
const int SCREEN_H = MAX * CELL_SIZE + 100; // Alto de ventana
...
// Union para guardar energia o simbolo
union InfoExtra {
    int energia;
    char simbolo;
};
...
// Estructura para cada paso del recorrido
struct Paso {
    int f, c, suma, chacales;
};

// Suma recursiva de los valores de los pasos
...
// Resuelve el recorrido diagonal y ultima columna
...
// Dibuja todo el laberinto y panel de informacion
...
// Genera nombre unico para el archivo de salida
...
int main() {
    ...
    vector<vector<int>> estados(MAX, vector<int>(MAX, 0)); // Estados de cada celda
    int posF, posC;
    vector<Paso> pasos = resolverLaberinto(M, posF, posC, estados); // Resuelve el laberinto
    ...
    int pasoIdx = 0;
    bool running = true;
    dibujarLaberinto8Bit(M, estados, pasoIdx, pasos[pasoIdx], font); // Dibuja el primer paso
    ...
    // Guarda resultados en archivo unico
